...
template<>
Xsection<GBHT, 3, double(*)(const double *, void*)>::
	Xsection(std::string Name, std::string configfile,
			double(*f)(const double*, void*)):
...
template<>
Xsection<GBHT, 4, double(*)(const double *, void*)>::
	Xsection(std::string Name, std::string configfile,
			double(*f)(const double*, void*)):
...
template<>
void Xsection<GBHT, 3, double(*)(const double*, void*)>::
	sample(std::vector<double> parameters,
			std::vector< fourvec > & FS){
	double sqrts = std::max(parameters[0], _mass*1.01), temp = parameters[1],
...
template<>
void Xsection<GBHT, 4, double(*)(const double*, void*)>::
	sample(std::vector<double> parameters,
			std::vector< fourvec > & FS){
...
/*------------------Implementation for 2 -> 3--------------------*/
template<>
scalar Xsection<GBHT, 3, double(*)(const double*, void*)>::
	find_max(std::vector<double> parameters){
	double sqrts = parameters[0], temp = parameters[1],
		   delta_t = parameters[2];
...
scalar Xsection<GB, 2, double(*)(const double*, void*)>::
	...
	double x2min = -std::log(1.+2./std::pow(temp/Qmax, 2)),
		   x2max = 0.;
	double Lx2 = x2max - x2min;
	auto dXdPS = [s, temp, Qmax, x2min, x2max, this](double * PS){
		double x2 = PS[2];
		double x[4] = {PS[0], PS[1],
					1.0 - (std::exp(-x2)-1.)*std::pow(temp/Qmax, 2), PS[3]};
		double Jacobian = std::exp(-x2)*std::pow(temp/Qmax, 2);
	...
	auto nega_dXdPS = [s, temp, Qmax, x2min, x2max, this](double * PS){
		double x2 = PS[2];
		double x[4] = {PS[0], PS[1],
					1.0 - (std::exp(-x2)-1.)*std::pow(temp/Qmax, 2), PS[3]};
		double Jacobian = std::exp(-x2)*std::pow(temp/Qmax, 2);
...
/*------------------Implementation for 3 -> 2--------------------*/
template<>
scalar Xsection<GBHT, 4, double(*)(const double*, void*)>::
	find_max(std::vector<double> parameters){
		double sqrts = parameters[0], temp = parameters[1],
			   xinel = parameters[2], yinel = parameters[3];
		double s = sqrts*sqrts;
...
template class Xsection<GBHT, 3, double(*)(const double*, void*)>;
template class Xsection<GBHT, 4, double(*)(const double*, void*)>;
