...
template <>
Rate<GBHT, 3, 3, double(*)(const double*, void *)>::
	Rate(std::string Name, std::string configfile, double(*f)(const double*, void *)):
StochasticBase<3>(Name+"/rate", configfile),
X(std::make_shared<Xsection<GBHT, 3, double(*)(const double*, void *)>>(Name, configfile, f) )
{
	// read configfile
	boost::property_tree::ptree config;
	std::ifstream input(configfile);
...
template <>
Rate<GBHT, 3, 4, double(*)(const double*, void *)>::
	Rate(std::string Name, std::string configfile, double(*f)(const double*, void *)):
StochasticBase<3>(Name+"/rate", configfile),
X(std::make_shared<Xsection<GBHT, 4, double(*)(const double*, void *)>>(Name, configfile, f) )
{
	// read configfile
	boost::property_tree::ptree config;
	std::ifstream input(configfile);
...
template <>
void Rate<GBHT, 3, 3, double(*)(const double*, void *)>::
		sample(std::vector<double> parameters,
			std::vector< fourvec > & final_states){
	double E = parameters[0];
	double T = parameters[1];
...
template <>
void Rate<GBHT, 3, 4, double(*)(const double*, void *)>::
		sample(std::vector<double> parameters,
			std::vector< fourvec > & final_states){
	double E = parameters[0];
	double T = parameters[1];
...
/*------------------Implementation for 2 -> 3--------------------*/
template <>
scalar Rate<GBHT, 3, 3, double(*)(const double*, void*)>::
		find_max(std::vector<double> parameters){
	double E = parameters[0];
	double T = parameters[1];
	double delta_t = parameters[2];
...
/*------------------Implementation for 3 -> 2--------------------*/
template <>
scalar Rate<GBHT, 3, 4, double(*)(const double*, void*)>::
		find_max(std::vector<double> parameters){
	double E = parameters[0];
	double T = parameters[1];
	double delta_t = parameters[2];
...
// For 2->3 with Gunion-Bertsch and Higher-twist like LPM effect
template class Rate<GBHT,3,3,double(*)(const double*, void*)>; 
// For 3->2 with Gunion-Bertsch and Higher-twist like LPM effect
template class Rate<GBHT,3,4,double(*)(const double*, void*)>;
