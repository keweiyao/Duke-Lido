...
// find max of f_p1
template <>
scalar OniumDissoRate23q<2, double(*)(double*, std::size_t, void*), double(*)(double, void*), double(*)(double, double)>:: //vector function: v, T, mQ, Enl, aB, prel_up, I_p1_max, max_p2Matrix, p2Matrix(prel, aB)
find_max(std::vector<double> parameters){
    double v = parameters[0];
    double T = parameters[1];
...
sample(std::vector<double> parameters, std::vector< fourvec > & FS){
    double v = parameters[0];
    double T = parameters[1];
    std::vector<double> pQpQbar_final = Sample_disso_ineq(v, T, _mass, _Enl, _aB, _prel_up, StochasticBase<2>::GetFmax(parameters).s, _max_p2Matrix, this->_f3);
...
sample(std::vector<double> parameters, std::vector< fourvec > & FS){
    ...
    double T = parameters[1];
    double p = std::exp(parameters[2]); // relative momentum in QQbar rest frame
    std::vector<double> ponium_final = S1S_reco_ineq(v, T, p, _mass, _Enl,
